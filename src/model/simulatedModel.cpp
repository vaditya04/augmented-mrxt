...
//	printf(">>>>>>>>>>>>>>>>>>\n");
    ...
    FILE* vInfoGain = 0;
    vInfoGain = fopen("/home/pkar/.mrxt/outfiles/tempLogIG.m", "w");
    fprintf(vInfoGain,"info_gain = [");
...
//        printf("%s\n", logstr);
        ...
        for(int r = 0; r < nBots; ++r)
        {
            ...
            int posy = gt[r].y*10;
            double range = 60;
            double senseangle = 2.0943951;
            IplImage *mask = cvCloneImage(coveredMap);
            for(int i = 0; i < mask->width; ++i)
            {
                for(int j = 0; j < mask->height; ++j)
                {
                    int x = i - posx;
                    int y = j - posy;
                    if((eucldist(x, y, 0, 0) < range) && (fabs(atan2(y,x) - gt[r].th) < senseangle))
                        continue;
                    returnPixel1C(mask, i, j) = 255;
                }
            }
            IplImage *clonemask = cvCloneImage(mask);
            cvFloodFill(clonemask, cvPoint(posx, posy), CV_RGB(255,255,255));
            cvSub(clonemask, mask, clonemask);
            int gain = cvCountNonZero(clonemask);
            tot_gain += gain;
            cvAdd(clonemask, coveredMap, coveredMap);
        }
        fprintf(vInfoGain,"%f, %d\n",elapsedTime,tot_gain);
//        cvShowImage("img", coveredMap);
...
void simulatedModel::loadMapFile(const ConfigFile& config){
    ...
    coveredMap = cvCreateImage(cvSize(400, 300), 8, 1);
    cvZero(coveredMap);
	for (int i = 0; i< numwalls; i++){
        ...
        CvPoint pt1 = cvPoint(x1*10, y1*10);
        CvPoint pt2 = cvPoint(x2*10, y2*10);
        cvLine(coveredMap, pt1, pt2, CV_RGB(255,255,255));
...
//	if (config.read<bool>("RANDOM")){
	if(mapLoaded){
        ...
        printf("aqui\n");
         printf("pose p: (%f, %f), in cells (%d, %d)\n", odo[0].x, odo[0].y, p.x, p.y);
...
double simulatedModel::eucldist(double x1, double y1, double x2, double y2)
{
