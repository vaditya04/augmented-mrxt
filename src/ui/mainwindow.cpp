...
void MainWindow::stopSimulation(bool){
    ...
    int nrobots = scene->getNumRobots();
    vector<double> distVec(nrobots);
    vector<double> posx(nrobots);
    vector<double> posy(nrobots);
    for(int i = 0; i < nrobots; i++)
    {
        posx[i] = scene->gtlogvector[0][i].x;
        posy[i] = scene->gtlogvector[0][i].y;
    }
    for(int i = 1; i < scene->gtlogvector.size(); i++)
    {
        for(int j = 0; j < nrobots; j++)
        {
            distVec[j] += scene->eucldist(scene->gtlogvector[i][j].x, scene->gtlogvector[i][j].y, posx[j], posy[j]);
            posx[j] = scene->gtlogvector[i][j].x;
            posy[j] = scene->gtlogvector[i][j].y;
        }
    }
    double distance = 0.0;
    ...
    for(int i = 0; i < nrobots; ++i)
    {
        qstr += "\n- Distance Travelled by R" + QString::number(i + 1) + " = " + QString::number(distVec[i]);
        distance += distVec[i];
    }
    qstr += "\n- Total Distance Travelled = " + QString::number(distance);
