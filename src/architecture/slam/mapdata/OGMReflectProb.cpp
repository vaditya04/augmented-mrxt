...
int OGMReflectProb::loadMapFromFile(const char* file){
	try{
		...
		if (inputfile.good()){
            ...
            cout<<width<<" "<<height<<" "<<realWidth<<" "<<realHeight<<" "<<xorigin<<" "<<yorigin<<" "<<resolution<<" "<<totalsize<<endl;
			if (cells) delete[] cells;
			cells = new cell[totalsize]();
            ...
            for(int j = 0; j < height ; ++j)
            {
                ...
                {
                    if(cells[j*width + i].val > 50.0f)
                    {
                        printf("%f\n", (cells[j*width + i].val));
                        returnPixel1C(img, i, j) = (int) (cells[j*width + i].val) *2.55;
                    }
                }
            }
            cvShowImage("img", img);
