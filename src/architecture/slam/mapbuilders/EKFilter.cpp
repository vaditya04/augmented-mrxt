...
EKFilter::EKFilter(int nrobots, ConfigFile& configFile):
    ...
    lastomap			(gridMapFactory::Instance().CreateObject(gmtype,width, height, resolution, xorigin, yorigin)),
	Identity3		(matrix::identity(3)),
...
//	
    ...
    IplImage *previnfo = cvCreateImage(cvSize(omap->getWidth(), omap->getHeight()), 8, 1);
    for(int i = 0; i < omap->getWidth(); i++)
    {
        for(int j = 0; j < omap->getHeight(); j++)
        {
            if(!omap->isunknown(i,j))
                ((uchar*)(previnfo->imageData + previnfo->widthStep*(j)))[i] = 255;
            else
                ((uchar*)(previnfo->imageData + previnfo->widthStep*(j)))[i] = 0;
        }
    }

    FILE *fp = fopen("/home/pkar/Desktop/data.txt", "a");
	while (!endSlam){
		//printID("SLAM EKF");
		paso++;
		// Leemos sensores
...
//        int lastInfo = 0, Info = 0;
//        for(int i = 0; i < omap->getWidth(); i++)
//        {
//            for(int j = 0; j < omap->getHeight(); j++)
//            {
//                if(omap->getValue(i, j))
//                    continue;
//                else
//                    Info++;
//            }
//        }
//        for(int i = 0; i < lastomap->getWidth(); i++)
//        {
//            for(int j = 0; j < lastomap->getHeight(); j++)
//            {
//                if(lastomap->getValue(i, j))
//                    continue;
//                else
//                    lastInfo++;
//            }
//        }
//        infoGain.push_back(Info - lastInfo);
//        printf("gain: %d\n", Info - lastInfo);
//        IplImage *image = cvCreateImage(cvSize(600, 600), 8, 1);
//        cvLine(image, cvPoint(timeCounter, 600), cvPoint(timeCounter, 600 - infoGain[timeCounter]), CV_RGB(255, 255, 255));
//        cvShowImage("infogain", image);
        IplImage *nextinfo = cvCreateImage(cvSize(omap->getWidth(), omap->getHeight()), 8, 1);
        for(int i = 0; i < omap->getWidth(); i++)
        {
            for(int j = 0; j < omap->getHeight(); j++)
            {
                if(!omap->isunknown(i,j))
                    ((uchar*)(nextinfo->imageData + nextinfo->widthStep*(j)))[i] = 255;
                else
                    ((uchar*)(nextinfo->imageData + nextinfo->widthStep*(j)))[i] = 0;
            }
        }
        timeCounter++;
		step();
		emit slamUpdated();
        IplImage *info = cvCreateImage(cvSize(omap->getWidth(), omap->getHeight()), 8, 1);
        cvSub(nextinfo, previnfo, info);
//        cvShowImage("info", info);
        cvMoveWindow("info", 200, 200);
        previnfo = nextinfo;
        fprintf(fp, "%d\n", cvCountNonZero(info));
		//if (paso%10==0){
		// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>  show maps  >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
